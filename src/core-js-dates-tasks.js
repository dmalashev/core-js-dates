...
function getNextFriday(date) {
  ...
  const diff = weekDay < 5 ? 5 - weekDay : (weekDay - 6) * -2 + weekDay;
...
function isDateInPeriod(date, period) {
  return (
    getCountDaysOnPeriod(period.start, date) > 0 &&
    getCountDaysOnPeriod(date, period.end) > 0
  );
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  while (actualMonth === month) {
    ...
    if ((date.getDay() === 6 || date.getDay() === 0) && actualMonth === month) {
      weekendCounter += 1;
    }
...
function getWeekNumberByDate(date) {
  const numberOfDay =
    (date - new Date(date.getFullYear(), 0)) / (1000 * 60 * 60 * 24) + 1;

  const dateDayOfWeek = date.getDay() ? date.getDay() : 7;
  let numberOfWeeks = Math.floor((numberOfDay - dateDayOfWeek) / 7);

  const janFirstDayNum = new Date(date.getFullYear(), 0, 1).getDay();

  if (janFirstDayNum !== 1) {
    numberOfWeeks += 1;
  }

  if (janFirstDayNum <= 4 && janFirstDayNum !== 0) {
    numberOfWeeks += 1;
  }
...
function getNextFridayThe13th(date) {
  let dayOf13;
  let month = date.getDate() < 13 ? date.getMonth() : date.getMonth() + 1;
  month -= 1;

  do {
    month += 1;
    dayOf13 = new Date(date.getFullYear(), month, 13);
  } while (dayOf13.getDay() !== 5);
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  do {
    date.setDate(date.getDate() + 1);
    if (!isDayoff) {
      datesArr.push(dateToString(date));
    }

    dayCounter += 1;
    if (isDayoff) {
      if (dayCounter === countOffDays) {
        dayCounter = 0;
        isDayoff = false;
      }
    } else if (dayCounter === countWorkDays) {
      dayCounter = 0;
      isDayoff = true;
    }
  } while (dateToString(date) !== period.end);
